...
import { X, Minus, Plus, ShoppingCart, ArrowRight } from "lucide-react"; // Cambié iconos
...
export const CartSidebar = () => {
  ...
  // Función para ir al Checkout y cerrar el menú
                  ...
                  <div className="flex flex-1 flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between mt-2">
                        <div className="flex items-center border rounded-md">
                            <button onClick={() => updateQuantity(item.variant.id, item.quantity - 1)} className="px-2 py-1 hover:bg-slate-100 border-r"><Minus className="h-3 w-3" /></button>
                            <span className="px-3 text-xs font-medium">{item.quantity}</span>
                            <button onClick={() => updateQuantity(item.variant.id, item.quantity + 1)} className="px-2 py-1 hover:bg-slate-100 border-l"><Plus className="h-3 w-3" /></button>
                        </div>
                        <div className="text-right">
                             <span className="block text-sm font-bold text-brand-primary">${(item.variant.price * item.quantity).toFixed(2)}</span>
                             <button onClick={() => removeItem(item.variant.id)} className="text-xs text-red-400 hover:text-red-600 underline mt-1">Eliminar</button>
                        </div>
                    </div>
                  </div>
                </div>
          ...
          {items.length > 0 && (
            <div className="border-t p-6 bg-slate-50 space-y-4">
                ...
                <Button 
                    ...
                    className="w-full bg-brand-primary hover:bg-brand-primary/90 text-white h-12 text-lg shadow-lg"
                >
